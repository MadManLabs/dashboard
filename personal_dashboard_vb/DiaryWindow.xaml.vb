...
Public Class DiaryWindow
    ...
    Private Sub Add_New_Diary_To_DB(ByVal Diary)
       ...
       "server=localhost;uid=root;password=password;database=vbdashboard"
        MysqlConn.Open()
        Try
            Dim Query As String = "insert into vbdashboard.diary (title,content,format,date,belong) values ('" + Diary.Title & "','" & Diary.Content & "','" & Diary.Format & "','" &
    ...
    Public Sub Update_Diaries_Data()
       ...
       "server=localhost;uid=root;password=password;database=vbdashboard"
        ...
        Try
            Dim Query As String = "select * from vbdashboard.diary where belong = " & UserData.Id
            COMMAND = New MySqlCommand(Query, MysqlConn)
    ...
    Private Sub Button_Click_1(sender As Object, e As RoutedEventArgs)
       ...
       "server=localhost;uid=root;password=password;database=vbdashboard"
        MysqlConn.Open()
        Try
            Dim Query As String = "delete from vbdashboard.diary where id = " & Integer.Parse(SelectedDetailDock.Tag.ToString)
            COMMAND = New MySqlCommand(Query, MysqlConn)
            COMMAND.ExecuteNonQuery()
            MysqlConn.Close()
        Catch ex As MySqlException
